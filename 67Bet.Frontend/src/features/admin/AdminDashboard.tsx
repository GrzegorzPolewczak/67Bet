...
import { ShieldCheck, Users, Activity, DollarSign, Check, X, Clock, Zap, Ticket, Plus, Power, PowerOff } from 'lucide-react';
import { motion, AnimatePresence } from 'framer-motion';
import toast from 'react-hot-toast';

const AdminDashboard: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
  const { pendingRequests, promoCodes, stats, loading } = useSelector((state: RootState) => state.admin);
  ...
  const handleCreatePromo = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(createPromoCodeAsync({ code: newPromoCode, reward })).unwrap();
      ...
      <div className="bg-gradient-to-r from-primary-900/40 to-transparent p-8 rounded-3xl border border-primary-500/20 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
        <div className="relative z-10">
          ...
          <p className="text-gray-400 text-sm max-w-xl leading-relaxed">
            Welcome to the administrator dashboard. Monitor platform activity, manage user requests, and oversee platform-wide promotional campaigns.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
        ...
        <section className="bg-dark-800/50 border border-dark-600 rounded-3xl overflow-hidden shadow-2xl flex flex-col">
          <div className="p-6 md:px-8 border-b border-dark-600 flex items-center justify-between bg-dark-800">
            ...
            <div className="bg-dark-900 border border-dark-600 px-4 py-1.5 rounded-full flex items-center gap-2">
              ...
              <span className="text-xs font-bold text-gray-300 uppercase tracking-widest">{pendingRequests.length}</span>
            </div>
          </div>
        ...
        <section className="bg-dark-800/50 border border-dark-600 rounded-3xl overflow-hidden shadow-2xl flex flex-col">
          <div className="p-6 md:px-8 border-b border-dark-600 flex items-center justify-between bg-dark-800">
            ...
            <div className="bg-dark-900 border border-dark-600 px-4 py-1.5 rounded-full flex items-center gap-2">
              <span className="text-xs font-bold text-gray-300 uppercase tracking-widest">{promoCodes.length} Codes</span>
            </div>
          </div>

          <div className="p-6 md:p-8 space-y-8 flex-1">
            {/* Create Promo Form */}
            <form onSubmit={handleCreatePromo} className="bg-dark-900 border border-dark-700 p-5 rounded-2xl space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest px-1">Code Name</label>
                  <input 
                    type="text"
                ...
                <div className="space-y-1.5">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest px-1">Freebet Reward ($)</label>
                  <input 
                    type="number"
                    step="0.01"
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${
                        promo.isActive ? 'bg-primary-600/20 text-primary-500' : 'bg-dark-700 text-gray-600'
                      }`}>
                        <Ticket className="w-5 h-5" />
                      </div>
                      <div>
                        <p className="text-sm font-black text-white tracking-wide">{promo.code}</p>
