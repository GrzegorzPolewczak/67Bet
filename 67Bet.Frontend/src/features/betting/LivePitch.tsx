...
const LivePitch: React.FC<LivePitchProps> = ({ sportKey, zone, action }) => {
  ...
  const getZoneOverlayClass = () => {
    switch (zone) {
      case 'HomeBox': return 'left-0 w-1/4 bg-red-500/20';
      case 'HomeDef': return 'left-0 w-1/2 bg-red-500/10';
      case 'Midfield': return 'left-1/4 w-1/2 bg-blue-500/10';
      case 'AwayDef': return 'right-0 w-1/2 bg-red-500/10';
      case 'AwayBox': return 'right-0 w-1/4 bg-red-500/20';
    ...
    if (isSoccer) {
      return (
        <svg className="w-full h-full" viewBox="0 0 100 60">
          ...
          <rect x="0" y="15" width="12" height="30" fill="none" stroke="white" strokeWidth="0.5" />
          <rect x="88" y="15" width="12" height="30" fill="none" stroke="white" strokeWidth="0.5" />
        </svg>
      );
    }
    if (isBasketball) {
      return (
        <svg className="w-full h-full" viewBox="0 0 100 60">
          ...
          {/* Trumny */}
          <rect x="0" y="20" width="19" height="20" fill="none" stroke="white" strokeWidth="0.8" />
          <rect x="81" y="20" width="19" height="20" fill="none" stroke="white" strokeWidth="0.8" />
          {/* Linie za 3 */}
          <path d="M 0 5 Q 40 30 0 55" fill="none" stroke="white" strokeWidth="0.8" />
          <path d="M 100 5 Q 60 30 100 55" fill="none" stroke="white" strokeWidth="0.8" />
        </svg>
      );
    }
    return (
  ...
  return (
    <div className={`relative w-full aspect-[16/9] rounded-2xl overflow-hidden border-2 border-dark-700 shadow-2xl transition-colors duration-700 ${isSoccer ? 'bg-green-950' : isBasketball ? 'bg-amber-950' : 'bg-dark-900'}`}>
      {/* Pitch Lines */}
        ...
        <AnimatePresence mode="wait">
          <motion.div
            ...
            className={`px-8 py-4 rounded-xl border-2 text-white font-black text-xl shadow-2xl uppercase tracking-tighter backdrop-blur-md ${
                action.includes('GOAL') || action.includes('POINTER') || action.includes('ELIMINATED') 
      ...
      <motion.div 
        animate={{ 
          x: zone.includes('Away') ? '80%' : zone.includes('Home') ? '15%' : '48%',
          y: '45%'
        }}
