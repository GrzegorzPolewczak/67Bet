    ...
    <div className="max-w-[1600px] w-full px-4 xl:px-8 mx-auto space-y-10">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 bg-dark-800 p-8 rounded-3xl border border-dark-700 shadow-2xl">
        <div>
          <div className="flex items-center gap-4 mb-3">
            ...
            <h1 className="text-3xl md:text-4xl font-black text-white italic tracking-tight">VIRTUAL RACING</h1>
          </div>
          <p className="text-gray-400 text-base">Experience the thrill of AI-generated horse racing 24/7 in our premium simulator.</p>
        </div>
      ...
      {loading ? (
        ...
        >
          ...
          <h2 className="text-3xl font-bold text-white mb-4">No Active Races</h2>
          <p className="text-gray-400 text-lg mb-8 max-w-xl mx-auto">
            The track is currently empty. Be the first to generate a new virtual race and watch the action unfold in ultra-high resolution!
          </p>
          <button
            onClick={generateRace}
            disabled={isGenerating}
              ...
              >
                <div className="bg-dark-900 p-6 border-b border-dark-700 flex justify-between items-center relative overflow-hidden">
                  ...
                  <div className="flex items-center gap-4 relative z-10">
                    ...
                    <div>
                      <h3 className="text-xl md:text-2xl font-black text-white leading-tight">{race.name}</h3>
                  ...
                  {runningRaces[race.id] ? (
                    ...
                    <div className="relative w-full h-[500px] bg-green-900/40 border-4 border-green-800/50 rounded-2xl overflow-hidden mb-8 flex flex-col justify-evenly py-4 shadow-inner">
                      ...
                      {race.participants.map((_, i) => (
                        <div key={i} className="absolute w-full h-[2px] bg-white/10" style={{ top: `${(i+1) * (100/(race.participants.length+1))}%` }} />
                      ))}
                      ...
                      {race.participants.map((pRaw, index) => {
                        ...
                        return (
                          <div key={p.id} className="relative z-10 flex items-center h-14 w-full">
                             <motion.div
                                initial={{ left: "5%" }}
                        ...
                        return (
                          <div 
                            ...
                            className={`flex items-center justify-between p-4 md:p-6 rounded-2xl transition-all border-2 cursor-pointer
                              ${isWinner ? 'bg-yellow-500/20 border-yellow-500 shadow-[0_0_20px_rgba(234,179,8,0.4)]' : 
                          ...
                          >
                            <div className="flex items-center gap-4 md:gap-6">
                              <span className={`w-8 h-8 md:w-10 md:h-10 rounded-lg flex items-center justify-center text-sm md:text-base font-black shadow-inner
                                ${isWinner ? 'bg-yellow-500 text-dark-900' : 'bg-dark-900 text-gray-400'}
                              `}>
                                {index + 1}
                              </span>
                              <div className="flex flex-col">
                                <span className={`font-black tracking-wide ${isWinner ? 'text-yellow-500 text-xl md:text-2xl' : 'text-gray-100 text-lg md:text-xl'}`}>
                                  {p.horseName}
                                </span>
                                <span className="text-xs md:text-sm text-gray-500 font-bold uppercase tracking-widest mt-0.5">Stallion / Skill Level 8</span>
                              </div>
                              {isWinner && <Trophy className="w-7 h-7 text-yellow-500 ml-2 animate-bounce" />}
                            </div>
                            
                            <div className={`px-6 py-3 rounded-xl font-black text-lg md:text-xl border-2 transition-colors shadow-md
