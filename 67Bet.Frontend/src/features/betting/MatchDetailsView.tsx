        ...
        {currentMatch?.streamUrl && (
          <div className="flex items-center gap-3">
            <div className="flex bg-dark-800 p-1 rounded-xl border border-dark-700">
              <button 
                onClick={() => { setViewMode('viz'); setIsPiP(false); }}
              ...
              <button 
                onClick={() => { setViewMode('stream'); setIsPiP(false); }}
          ...
          >
             <iframe
              ...
              <div className="absolute top-2 right-2 flex gap-1">
                 <button onClick={() => setIsPiP(false)} className="bg-black/50 hover:bg-red-500 p-1.5 rounded-lg text-white transition-colors">
        ...
        <div className="xl:col-span-2 space-y-6">
           <div className="bg-dark-800 border border-dark-700 rounded-3xl p-6 overflow-hidden">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full animate-pulse ${viewMode === 'stream' ? 'bg-red-500' : 'bg-primary-500'}`} />
                  <h2 className="text-sm font-bold text-white uppercase tracking-wider">
                    {viewMode === 'stream' ? 'Live Transmission' : 'Live Action Tracker'}
                  </h2>
                </div>
                {currentMatch && (
              ...
              <div className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-8 items-center border-t border-dark-700 pt-8">
                 <div className="space-y-4">
                   <div className="flex justify-between items-end">
                      <h3 className="text-[10px] font-black text-gray-500 uppercase tracking-widest">Pressure Momentum</h3>
                      <div className="text-[10px] font-bold text-primary-400">{currentMatch?.momentum}%</div>
                   </div>
                 ...
                 <StatsBars sportKey={event.sportKey} stats={currentMatch?.statistics || {}} />
              </div>
           </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Array.isArray(event.markets) && event.markets.length > 0 ? (
                event.markets.map((market: any, mIndex: number) => (
                  <div key={market.id || mIndex} className="bg-dark-900 p-5 rounded-2xl border border-dark-700 group hover:border-primary-500/30 transition-all duration-300 shadow-sm hover:shadow-primary-500/5">
                    <h3 className="text-[10px] font-black text-gray-500 uppercase mb-4 tracking-widest group-hover:text-primary-400 transition-colors">{market.name}</h3>
