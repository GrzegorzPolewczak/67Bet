...
const MatchTimeline: React.FC<MatchTimelineProps> = ({ events }) => {
  if (!events || events.length === 0)
    return (
      <div className="text-gray-600 italic text-sm text-center py-4">
        No events yet...
      </div>
    );
  ...
  return (
    <div className="space-y-3 max-h-[300px] overflow-y-auto pr-2 custom-scrollbar">
      {events.map((evt, idx) => (
        ...
        >
          <div className="text-xs font-black text-primary-500 w-8">
            {evt.minute}
          </div>
          <div className="flex-1">
            <p
              className={`text-sm font-bold ${evt.type === "Goal" ? "text-white" : "text-gray-300"}`}
            >
              {evt.description}
            </p>
          </div>
