    ...
    <div className="space-y-5">
      ...
      {isBasketball && (
        <>
          {renderComparisonBar(
            "Field Goals",
            stats["FGPercentHome"] || 45,
            stats["FGPercentAway"] || 42,
            "%",
          )}
          <div className="grid grid-cols-2 gap-3">
      ...
      {isTennis && (
        <>
          {renderComparisonBar(
            "Win Probability",
            stats["WinProbHome"] || 50,
            100 - (stats["WinProbHome"] || 50),
            "%",
          )}
          <div className="grid grid-cols-2 gap-3">
        ...
        <>
          {sportKey.includes("csgo") ? (
            <>
              {renderComparisonBar(
                "Team Economy",
                stats["EconomyHome"] || 50,
                stats["EconomyAway"] || 48,
                "$k",
              )}
              <div className="grid grid-cols-2 gap-3">
                <StatBox
                  label="Rounds"
                  val={`${stats["RoundsHome"] || 0} - ${stats["RoundsAway"] || 0}`}
                  isString
                />
                <StatBox
                  label="Bomb Status"
                  val={stats["BombPlanted"] === 1 ? "PLANTED" : "CLEAR"}
                  isString
                />
              </div>
            </>
...
const StatBox = ({
  ...
  <div className="bg-dark-900/40 p-2.5 rounded-xl border border-dark-700/50 text-center hover:bg-dark-800 transition-colors">
    <span className="block text-[9px] text-gray-500 font-black uppercase mb-0.5 tracking-tighter">
      {label}
    </span>
