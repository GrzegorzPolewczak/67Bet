...
const RouletteSlider: React.FC<RouletteSliderProps> = ({
  spinAnim,
  targetNumber,
  onSpinEnd,
}) => {
  ...
  useEffect(() => {
    if (spinAnim && targetNumber !== null && containerRef.current) {
      ...
      let targetIndex = newStrip.findIndex(
        (n, idx) => n === targetNumber && idx > 100,
      );
      ...
      const targetX =
        containerWidth / 2 -
        (targetIndex * ITEM_WIDTH + ITEM_WIDTH / 2) +
        randomOffset;
  ...
  return (
    <div
      ref={containerRef}
      className="relative w-full h-32 bg-dark-900 rounded-2xl border border-dark-700 overflow-hidden shadow-inner flex items-center"
    >
      {/* Center Pointer */}
