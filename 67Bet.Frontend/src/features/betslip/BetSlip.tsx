    ...
    <div className="flex flex-col h-full bg-dark-800">
      <div className="p-4 border-b border-dark-700 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <h2 className="font-bold text-sm uppercase tracking-wider">Bet Slip</h2>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {selections.length === 0 ? (
          <div className="h-full flex flex-col items-center justify-center text-center p-6 space-y-4">
            ...
            <div>
              <p className="text-sm font-bold text-gray-300">Your slip is empty</p>
              <p className="text-xs text-gray-500 mt-1">Add events to start betting</p>
            </div>
          </div>
        ) : (
          <AnimatePresence>
            {selections.map((selection) => (
              ...
              >
                ...
                <div className="pr-6">
                  <p className="text-[10px] font-bold text-primary-500 uppercase">{selection.marketName}</p>
                  <p className="text-xs font-bold mt-0.5">{selection.outcomeName}</p>
                  <p className="text-[10px] text-gray-400 mt-1">{selection.eventName}</p>
                </div>
      ...
      {selections.length > 0 && (
        <div className="p-4 bg-dark-700/50 border-t border-dark-700 space-y-4">
          ...
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-gray-500 uppercase px-1">Stake</label>
