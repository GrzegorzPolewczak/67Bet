...
const ResponsibleGamblingPage: React.FC = () => {
  ...
  const [validationMode, setValidationMode] = useState<"stake" | "deposit">(
    "stake",
  );
  ...
  const validateAmount = async () => {
    ...
    try {
      const endpoint =
        validationMode === "stake" ? "validate-stake" : "validate-deposit";
      const response = await bettingApi.post(
        `/responsible-gambling/me/${endpoint}`,
        {
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6">
          ...
          <div className="space-y-4">
            ...
            <UsageRow
              label="Daily deposit"
              used={dashboard?.usage.dailyDepositUsed}
              remaining={dashboard?.usage.dailyDepositRemaining}
            />
            <UsageRow
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6">
          ...
          <input
            ...
            onChange={(event) =>
              setValidationAmount(Number(event.target.value))
            }
            className="w-full bg-dark-900 border border-dark-600 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500"
          />
          <button
            onClick={validateAmount}
            className="w-full mt-5 h-12 rounded-xl bg-dark-700 hover:bg-dark-600 text-white font-black"
          >
            Check Amount
          </button>
          {validationResult && (
            ...
            >
              ...
              {validationResult.message && (
                <p className="text-sm mt-2">{validationResult.message}</p>
              )}
              {validationResult.blockedUntilUtc && (
                <p className="text-sm mt-2">
                  Until {formatDate(validationResult.blockedUntilUtc)}
                </p>
              )}
            </div>
          )}
        ...
        <div className="bg-dark-800 border border-dark-700 rounded-2xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1fr_auto] gap-3">
            <select
              value={activityType}
              onChange={(event) =>
                setActivityType(Number(event.target.value) as ActivityType)
              }
            ...
            <input
              ...
              onChange={(event) =>
                setActivityAmount(Number(event.target.value))
              }
              className="bg-dark-900 border border-dark-600 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-cyan-500"
            />
            <button
              onClick={recordActivity}
              className="h-12 px-6 rounded-xl bg-cyan-600 hover:bg-cyan-500 text-dark-900 font-black"
            >
              Record
            </button>
          </div>
